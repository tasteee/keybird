...
function parseScaleInput(input: string): { tonic: string; scaleType: string } {
	...
	const lowerScaleType = scaleType.toLowerCase() as keyof typeof SCALE_TYPES

	if (!SCALE_TYPES[lowerScaleType]) {
...
function getScaleNotes(tonic: string, scaleType: string): string[] {
	const lowerScaleType = scaleType.toLowerCase() as keyof typeof SCALE_TYPES
	const tonalScaleType = SCALE_TYPES[lowerScaleType]
 ...
 */
function generateChordsForRoot(root: string, scaleNotes: string[]): TonalChordT[] {
	const validChords: TonalChordT[] = []
		...
		try {
			...
			if (chord.notes && chord.notes.length > 0) {
				if (chordFitsInScale(chord.notes, scaleNotes)) {
					...
					const adjustedNotes = getAdjustedNotes(base)
