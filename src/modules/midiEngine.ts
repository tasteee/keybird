...
export const createMidiEngine = (): MidiEngineT => {
	...
	const updateLivePlayback = just.debounce(150, () => {
		...
		const midiSignals: MidiT.Signal[] = []
		...
		store.progression.steps.forEach((step, stepIndex) => {
			...
			store.signals.forEach((signal: SignalT) => {
				const note = toneMap[signal.toneId]
				const startTicks = stepStartTicks + theory.getDivisionsTicks(signal.startDivision)
				const endTicks = stepStartTicks + theory.getDivisionsTicks(signal.endDivision)
	...
	const scheduleMidiPlayback = () => {
		...
		const isOutputMidi = $output.outputType === 'midi'
		const isOutputInstrument = $output.outputType === 'instrument'
		...
		if (isOutputInstrument) {
			const instrumentKey = $player.instrumentName as InstrumentName
			const instrument = $player.loadedInstruments[instrumentKey]
...
const generateToneMapForChord = (chord: TonalChordT): Record<string, number> => {
