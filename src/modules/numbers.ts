...
// randomInt(min, max) // number between min and max
// randomInt({ min, max }) // number between min and max
// randomInt(100) // min 0 max 100
// randomInt(-100) // min -100 max 0
// randomInt() // min 0 max 666666
const randomInt = (...args: any[]): number => {
	if (args.length === 2) {
	...
	if (args.length === 1 && typeof args[0] === 'object') {
		const { min, max } = args[0] as { min: number; max: number }
		return randomUtil.int(min, max)
	}

	if (args.length === 1 && typeof args[0] === 'number') {
		const isArgLow = args[0] < 0
		const firstArg = isArgLow ? args[0] : 0
		const secondArg = isArgLow ? 0 : args[0]
		return randomUtil.int(firstArg, secondArg)
	}
}
