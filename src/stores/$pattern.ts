...
import { DIVISIONS_PER_BEAT } from '#/constants'
...
class PatternStore {
	...
	@computed get activeTones() {
		const list = Object.values($pattern.toneMap) as ToneT[]
		const filter = (tone: ToneT) => tone.octave === $pattern.octave
	...
	@computed get activeToneIds() {
		...
		const sorted = $pattern.activeTones.sort(toneIndexSort)
	...
	@action toggleRowSignalId = (args: ToggleRowSignalIdArgsT) => {
		const row = this.toneMap[args.toneId]
		const hasSignal = row.signalIds.includes(args.signalId)
		const idFilter = (id: string) => id !== args.signalId
		const newSignals = hasSignal ? row.signalIds.filter(idFilter) : [...row.signalIds, args.signalId]
		const updatedRow = { ...row, signalIds: newSignals }
		this.toneMap[args.toneId] = updatedRow
	}
	...
	@action addSignal = (signal: Partial<SignalT>) => {
		...
		const fallbackEndDivision = signal.startDivision + DIVISIONS_PER_BEAT
		const endDivision = signal.endDivision ?? fallbackEndDivision
		const durationDivisions = endDivision - signal.startDivision
		...
		this.toggleRowSignalId({ toneId: signal.toneId, signalId: id })
	}

	@action removeSignal = (signalId: string) => {
		const signal = this.signalMap[signalId]
		delete this.signalMap[signalId]
		this.toggleRowSignalId({ toneId: signal.toneId, signalId })
	}

	@action moveSignal = (args: MoveSignalArgsT) => {
		const updatedDate = Date.now()
		const signal = { ...this.signalMap[args.id] }
		const oldRowId = signal.toneId
		const newRowId = args.toneId
		const shouldMoveRows = oldRowId !== newRowId

		const updates: PartialSignalWithIdT = {
			id: args.id,
			toneId: newRowId,
			startDivision: args.startDivision ?? signal.startDivision,
			endDivision: args.endDivision ?? signal.endDivision,
			updatedDate
		}

		if (shouldMoveRows) {
			this.toggleRowSignalId({ toneId: oldRowId, signalId: signal.id })
			this.toggleRowSignalId({ toneId: newRowId, signalId: args.id })
		}

		this.updateSignal(updates)
	}

	@action updateSignal = (args: PartialSignalWithIdT) => {
		const id = args.id!
